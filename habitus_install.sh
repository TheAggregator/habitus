...
do_install() {
  if is_os "darwin"; then
    ...
    `curl -L --progress-bar -o /tmp/habitus https://github.com/theaggregator/habitus/releases/download/$version/habitus_darwin_amd64`
  elif is_os "linux"; then
    if [[ is_64 ]]; then
	  e_info "Downloading Habitus for Linux x64..."
	  `rm -f /tmp/habitus &> /dev/null`
	  `curl -L --progress-bar -o /tmp/habitus https://github.com/theaggregator/habitus/releases/download/$version/habitus_linux_amd64`
    else
	  e_info "Downloading Habitus for Linux x32..."
	  `rm -f /tmp/habitus &> /dev/null`
	  `curl -L --progress-bar -o /tmp/habitus https://github.com/theaggregator/habitus/releases/download/$version/habitus_linux_386`
    fi
  else
